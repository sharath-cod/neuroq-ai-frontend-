...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-[#0a0f1e] text-slate-100 overflow-hidden">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} transition-all duration-300 bg-[#0d1526] border-r border-cyan-900/30 flex flex-col flex-shrink-0`}>
        {/* Logo */}
        <div className="p-4 border-b border-cyan-900/30 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center flex-shrink-0">
            <Brain size={18} className="text-white" />
          </div>
          {sidebarOpen && (
            <div>
              <div className="font-bold text-sm text-cyan-400 tracking-wider">NeuroQ AI</div>
              <div className="text-xs text-slate-500">Neural Diagnostics</div>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map(({ to, icon: Icon, label }) => (
            ...
            >
              {({ isActive }) => <>
                <Icon size={18} className={`flex-shrink-0 ${isActive ? 'text-cyan-400' : ''}`} />
                {sidebarOpen && <span className="text-sm font-medium">{label}</span>}
                {sidebarOpen && isActive && <ChevronRight size={14} className="ml-auto text-cyan-400" />}
                {!sidebarOpen && (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="h-14 bg-[#0d1526]/80 backdrop-blur border-b border-cyan-900/30 flex items-center justify-between px-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1.5 rounded-lg hover:bg-white/5 text-slate-400">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)}
                className="relative p-2 rounded-lg hover:bg-white/5 text-slate-400 hover:text-slate-200">
                <Bell size={18} />
                {unread > 0 && (
              ...
              {notifOpen && (
                <div className="absolute right-0 top-10 w-80 bg-[#1a2332] border border-cyan-900/30 rounded-xl shadow-2xl z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-cyan-900/20 flex items-center justify-between">
                    <span className="text-sm font-semibold">Notifications</span>
                    <span className="text-xs text-cyan-400 cursor-pointer" onClick={() => { /* markAllRead */ }}>Mark all read</span>
                  </div>
                  <div className="max-h-72 overflow-y-auto divide-y divide-cyan-900/10">
                    ...
                    {notifications.map(n => (
                      <div key={n.id} onClick={() => markRead(n.id)}
                        className={`px-4 py-3 cursor-pointer hover:bg-white/5 ${!n.is_read ? 'bg-cyan-500/5' : ''}`}>
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-600 mt-1">{new Date(n.created_at).toLocaleString('en-IN')}</div>
                          </div>
                        </div>
                      </div>
                    ))}
