...
function QubitWire({ label, amplitude, gate, color }) {
  return (
    <div className="flex items-center gap-2 mb-2">
      <div className="text-xs font-mono text-slate-400 w-20 text-right">{label}</div>
      <div className="flex-1 flex items-center gap-1 relative">
        {/* Wire line */}
        <div className="absolute inset-x-0 top-1/2 h-px bg-slate-100"/>
        {/* Gate */}
        <div className="relative z-10 w-8 h-7 border border-slate-600 bg-white rounded flex items-center justify-center text-[9px] font-mono flex-shrink-0" style={{color}}>
          {gate}
        </div>
        {/* Amplitude visualization */}
        <div className="relative z-10 flex-1 h-2 bg-white rounded-full mx-2 overflow-hidden">
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Quantum Prediction Engine</h1>
          <p className="text-sm text-slate-400">Variational Quantum Circuit (VQC) simulator</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
          <h3 className="text-sm font-semibold text-slate-400 mb-4">Disease Circuit Selection</h3>
          ...
          {result && (
            <div className="mt-4 p-4 rounded-xl text-center" style={{background:RISK_COLORS[result.label]+'15', border:`1px solid ${RISK_COLORS[result.label]}30`}}>
              ...
              <div className="text-xs text-slate-400 mt-1">Entanglement strength: {result.entangle}</div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
          ...
          <div className="border-t border-slate-200 pt-4">
            <div className="text-xs text-slate-400 mb-2">Circuit info</div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-slate-50 rounded-lg p-2"><span className="text-slate-400">Qubits:</span> <span className="text-slate-400 font-mono">{selected.qubits.length}</span></div>
              <div className="bg-slate-50 rounded-lg p-2"><span className="text-slate-400">Gates:</span> <span className="text-slate-400 font-mono">CNOT+Ry+Rz</span></div>
              <div className="bg-slate-50 rounded-lg p-2"><span className="text-slate-400">Layers:</span> <span className="text-slate-400 font-mono">3</span></div>
              <div className="bg-slate-50 rounded-lg p-2"><span className="text-slate-400">Shots:</span> <span className="text-slate-400 font-mono">1024</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Explanation */}
      <div className="mt-5 bg-white border border-purple-500/20 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-purple-400 mb-3">How the Quantum Engine Works</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-slate-400">
          <div><div className="text-slate-400 font-medium mb-1">1. Feature Encoding</div>Each biomarker (Amyloid, Tau, etc.) is normalized and encoded as a qubit amplitude using rotation gates (Ry, Rz, Rx).</div>
          <div><div className="text-slate-400 font-medium mb-1">2. Quantum Entanglement</div>CNOT gates create entanglement between correlated biomarkers (e.g., Amyloid×Tau), capturing combined effects classical models miss.</div>
          <div><div className="text-slate-400 font-medium mb-1">3. Measurement & Fusion</div>Qubit amplitudes are measured and fused with a classical neural sigmoid layer to produce the final risk percentage.</div>
        </div>
      </div>
    </div>
  );
