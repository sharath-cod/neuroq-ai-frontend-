...
export default function AIPredict() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">AI Disease Prediction</h1>
        <p className="text-sm text-slate-400 mt-1">Multi-disease quantum-AI neural network analysis</p>
      </div>

      {/* Input */}
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 mb-6">
        <h3 className="text-sm font-semibold text-slate-400 mb-4">Run Prediction</h3>
        <div className="flex gap-3">
          <div className="relative flex-1">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
            <input value={patientId} onChange={e=>setPatientId(e.target.value)}
              placeholder="Enter Patient ID (1–8 for demo)"
              className="w-full pl-9 pr-4 py-2.5 bg-[#f0f4f8] border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-600 focus:outline-none focus:border-blue-400" />
          </div>
        ...
        <div className="flex gap-2 mt-3">
          {[1,2,3,4,5,6,7,8].map(n=>(
            <button key={n} onClick={()=>setPatientId(String(n))}
              className="w-8 h-8 rounded-lg bg-slate-50 border border-white/10 text-xs text-slate-400 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300 transition-all">
              {n}
            </button>
          ))}
        </div>
      ...
      {result && (
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-semibold text-slate-700">Results for {result.patient?.name}</h2>
            <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">{result.patient?.code}</span>
          </div>

          {/* Disease grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(result.predictions).map(([code, pred]) => (
              <div key={code} className={`bg-white border rounded-xl p-4 ${pred.score>=70?'border-red-500/30':pred.score>=50?'border-yellow-500/20':'border-slate-200'}`}>
                <div className="text-xs text-slate-400 mb-2">{DISEASE_LABELS[code]}</div>
                ...
                <div className="text-xs text-slate-400 mt-2">{pred.stage}</div>
                <div className="text-xs text-slate-400">Conf: {pred.confidence}%</div>
              </div>
            ))}
          </div>
          ...
          {result.shap && (
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <h3 className="text-sm font-semibold text-slate-400 mb-4">SHAP Feature Importance — {DISEASE_LABELS[result.primary_disease]}</h3>
          ...
          <button onClick={()=>navigate(`/patients/${patientId}`)}
            className="px-5 py-2.5 bg-blue-50 border border-blue-200 text-blue-600 rounded-xl text-sm hover:bg-blue-100 transition-all">
