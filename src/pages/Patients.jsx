    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">{total} total patients in system</p>
        </div>
        <button onClick={() => navigate('/patients/add')}
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-xl text-sm font-medium hover:from-cyan-400 hover:to-blue-500 transition-all shadow-lg shadow-cyan-500/20">
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl shadow-sm text-sm text-slate-400 placeholder-slate-600 focus:outline-none focus:border-blue-400 transition-all" />
        </div>
        <div className="relative">
          <Filter size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <select value={status} onChange={e=>{setStatus(e.target.value);setPage(1);}}
            className="pl-9 pr-8 py-2.5 bg-white border border-slate-200 rounded-xl shadow-sm text-sm text-slate-400 focus:outline-none focus:border-blue-400 appearance-none cursor-pointer">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-cyan-900/10">
              ...
              {!loading && patients.length === 0 && (
                <tr><td colSpan={7} className="py-12 text-center text-sm text-slate-400">No patients found</td></tr>
              )}
              {!loading && patients.map(p => (
                <tr key={p.id} className="hover:bg-white/2 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-200 to-blue-300 border border-blue-200 flex items-center justify-center text-sm font-bold text-blue-600 flex-shrink-0">
                        {p.full_name?.charAt(0)}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-slate-700">{p.full_name}</div>
                        <div className="text-xs text-slate-400 font-mono">{p.patient_code}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-sm text-slate-400">{p.age}y</div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => navigate(`/patients/${p.id}`)}
                        className="p-1.5 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-all" title="View">
                        <Eye size={15}/>
                      </button>
                      <button onClick={() => handleDelete(p.id, p.full_name)}
                        className="p-1.5 rounded-lg hover:bg-red-500/15 text-slate-400 hover:text-red-400 transition-all" title="Delete">
                        <Trash2 size={15}/>
                      </button>
                    </div>
                  </td>
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200">
            <span className="text-xs text-slate-400">Page {page} of {pages} · {total} records</span>
            <div className="flex gap-2">
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                className="p-1.5 rounded-lg hover:bg-slate-100 disabled:opacity-30 text-slate-400">
                <ChevronLeft size={16}/>
              </button>
