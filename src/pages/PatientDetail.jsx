...
export default function PatientDetail() {
  ...
  if (!patient) return <div className="p-6 text-slate-400">Patient not found</div>;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={()=>navigate('/patients')} className="p-2 rounded-xl hover:bg-slate-100 text-slate-400"><ArrowLeft size={18}/></button>
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-800">{p.full_name}</h1>
            <span className="text-xs font-mono text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">{p.patient_code}</span>
          ...
          <p className="text-sm text-slate-400 mt-0.5">{p.age}y · {p.gender} · {p.blood_group||'Blood group N/A'} · {p.phone||'No phone'}</p>
        </div>
      ...
      {/* Risk score hero */}
      {latestPred && (
        <div className="bg-white border rounded-xl p-5 mb-5 flex items-center gap-6" style={{borderColor: riskColor+'30'}}>
          <div className="text-center">
            <div className="text-5xl font-bold font-mono" style={{color: riskColor}}>{latestPred.risk_score}%</div>
            <div className="text-xs text-slate-400 mt-1">{latestPred.risk_label} Risk</div>
          </div>
          <div className="h-16 w-px bg-slate-100"/>
          <div className="flex-1 grid grid-cols-3 gap-4">
            <div><div className="text-xs text-slate-400">Primary Disease</div><div className="text-sm font-medium text-slate-700 mt-1">{latestPred.disease_name}</div></div>
            <div><div className="text-xs text-slate-400">Disease Stage</div><div className="text-sm font-medium text-slate-700 mt-1">{latestPred.disease_stage}</div></div>
            <div><div className="text-xs text-slate-400">AI Confidence</div><div className="text-sm font-medium text-slate-700 mt-1">{latestPred.confidence}%</div></div>
          </div>
        </div>
      )}

      {/* Tabs */}
      <div className="flex gap-1 mb-5 bg-white border border-slate-200 rounded-xl shadow-sm p-1">
        {TABS.map(t=>(
          <button key={t} onClick={()=>setTab(t)}
            className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium capitalize transition-all ${tab===t?'bg-blue-100 text-blue-600':'text-slate-400 hover:text-slate-400'}`}>
            {t.replace('-',' ')}
          </button>
        ))}
      </div>
      ...
      {tab==='overview' && (
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={l} className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
              <div className="text-xs text-slate-400 mb-2 uppercase tracking-wider">{l}</div>
          ...
          {latestPred?.suggested_treatment && (
            <div className="col-span-2 bg-blue-50 border border-blue-200 rounded-xl p-4">
              <div className="text-xs text-blue-600 mb-2 uppercase tracking-wider font-medium">AI Suggested Treatment</div>
              <div className="text-sm text-slate-400">{latestPred.suggested_treatment}</div>
            </div>
          )}
        </div>
      )}
      ...
      {tab==='biomarkers' && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={b.l} className={`bg-white border rounded-xl p-4 ${b.high?'border-red-500/30':'border-slate-200'}`}>
              <div className="text-xs text-slate-400 mb-1">{b.l}</div>
              <div className={`text-xl font-bold font-mono ${b.high?'text-red-400':'text-green-400'}`}>{b.v ?? '—'} <span className="text-xs font-normal text-slate-400">{b.u}</span></div>
              <div className="text-xs text-slate-400 mt-1">Normal: {b.normal}</div>
              {b.v && <div className="mt-2 h-1 rounded-full bg-white"><div className="h-full rounded-full" style={{width:`${Math.min(100,(b.v/(parseFloat(b.normal.replace(/[<>]/,''))||1))*50)}%`,background:b.high?'#f85149':'#3fb950'}}/></div>}
            </div>
          ))}
        </div>
      )}
      ...
      {tab==='genetics' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={g.l} className={`bg-white border rounded-xl p-4 text-center ${g.v?'border-red-500/30':'border-slate-200'}`}>
              <div className="text-xs text-slate-400 mb-2">{g.l}</div>
          ...
          <div className="bg-white border border-yellow-500/20 rounded-xl p-4 text-center">
            <div className="text-xs text-slate-400 mb-2">HTT CAG Repeats</div>
            <div className={`text-xl font-bold font-mono ${(p.genetics?.htt_cag||0)>36?'text-red-400':'text-green-400'}`}>{p.genetics?.htt_cag||18}</div>
            <div className="text-xs text-slate-400 mt-1">{(p.genetics?.htt_cag||0)>36?'⚠ Risk (>36)':'Normal range'}</div>
          </div>
        </div>
      )}
      ...
      {tab==='visits' && (
        <div className="space-y-3">
          {p.visits?.length === 0 && <div className="text-center py-10 text-slate-400">No visits recorded</div>}
          {p.visits?.map((v,i)=>(
            <div key={i} className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-slate-700">{new Date(v.visit_date).toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}</span>
                  <span className="text-xs capitalize text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{v.visit_type}</span>
                </div>
              </div>
              <div className="grid grid-cols-4 md:grid-cols-6 gap-3">
                {[{l:'MMSE',v:v.mmse,max:30},{l:'MoCA',v:v.moca,max:30},{l:'UPDRS Motor',v:v.updrs_motor,max:108},{l:'ALSFRS',v:v.alsfrs,max:48},{l:'EDSS',v:v.edss,max:10},{l:'Memory',v:v.memory_score,max:30}].map(s=>(
                  <div key={s.l} className="text-center">
                    ...
                    <div className="text-sm font-bold text-slate-700 font-mono">{s.v ?? '—'}</div>
                    {s.v && <div className="text-xs text-slate-400">/{s.max}</div>}
                  </div>
                ))}
              </div>
              {v.notes && <div className="mt-3 text-xs text-slate-400 bg-slate-50 rounded-lg p-2">{v.notes}</div>}
            </div>
          ))}
        </div>
      )}
        ...
        <div className="space-y-3">
          {p.predictions?.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400">No predictions yet. Click "Run AI Predict" to start.</p>
            </div>
          )}
            ...
            <div key={i} className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700">{pred.disease_name}</span>
                  <span className="text-xs text-slate-400">{pred.disease_stage}</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-xl font-bold font-mono" style={{color:RISK_COLORS[pred.risk_label]}}>{pred.risk_score}%</div>
                    <div className="text-xs text-slate-400">{pred.risk_label}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium text-slate-400">{pred.confidence}%</div>
                    <div className="text-xs text-slate-400">Confidence</div>
                  </div>
                </div>
              </div>
              {pred.suggested_treatment && (
                <div className="mt-3 text-xs text-slate-400 bg-slate-50 rounded-lg p-3">{pred.suggested_treatment}</div>
              )}
              <div className="text-xs text-slate-400 mt-2">{new Date(pred.predicted_at).toLocaleString('en-IN')}</div>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-slate-400">NeuroQ Clinical AI</span>
        <span className="text-xs text-slate-400">· Mistral-7B via HuggingFace</span>
      </div>
      <div className="h-72 overflow-y-auto p-4 space-y-3">
        {messages.map((m,i)=>(
          <div key={i} className={`flex gap-2 ${m.role==='user'?'flex-row-reverse':''}`}>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${m.role==='user'?'bg-blue-100 text-blue-600':'bg-purple-500/20 text-purple-400'}`}>
              {m.role==='user'?'U':'🧠'}
            </div>
            <div className={`max-w-[80%] px-3 py-2 rounded-xl text-xs leading-relaxed ${m.role==='user'?'bg-blue-50 text-cyan-100':'bg-slate-50 text-slate-400'}`}>
              {m.content}
            </div>
          </div>
        ))}
      ...
      <div className="flex flex-wrap gap-2 px-4 pb-2">
        {QUICK.map(q=><button key={q} onClick={()=>setInput(q)} className="text-xs px-2 py-1 rounded-full bg-slate-50 border border-white/10 text-slate-400 hover:text-slate-700 transition-all">{q}</button>)}
      </div>
      <div className="flex gap-2 p-3 border-t border-slate-200">
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}
          placeholder="Ask about this patient's condition…"
          className="flex-1 bg-[#f0f4f8] border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 placeholder-slate-600 focus:outline-none focus:border-blue-500/40"/>
