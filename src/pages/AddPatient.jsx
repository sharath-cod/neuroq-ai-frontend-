...
function Section({ icon: Icon, title, children }) {
  return (
    <div className="bg-[#0d1526] border border-cyan-900/20 rounded-xl p-5">
      <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-300 mb-5 pb-3 border-b border-cyan-900/20">
        <Icon size={16} className="text-cyan-400" /> {title}
      </h3>
...
const inp = "w-full bg-[#0a0f1e] border border-cyan-900/30 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-cyan-500/50 transition-all";
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={()=>navigate('/patients')} className="p-2 rounded-xl hover:bg-white/5 text-slate-400">
          <ArrowLeft size={18}/>
        </button>
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Add New Patient</h1>
          <p className="text-sm text-slate-500">Complete the form to register a patient in NeuroQ AI</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-[#0d1526] border border-cyan-900/20 rounded-xl p-5">
          <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-300 mb-5 pb-3 border-b border-cyan-900/20">
            <Upload size={16} className="text-cyan-400" /> Medical Files
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={key}>
                ...
                <label className="flex flex-col items-center gap-2 p-4 border-2 border-dashed border-cyan-900/40 rounded-xl cursor-pointer hover:border-cyan-500/40 transition-all">
                  <Upload size={20} className="text-slate-500" />
                  <span className="text-xs text-slate-500">{files[key] ? files[key].name : 'Click to upload (PDF/IMG, max 20MB)'}</span>
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={()=>navigate('/patients')}
            className="px-5 py-2.5 border border-slate-700 text-slate-400 rounded-xl text-sm hover:bg-white/5 transition-all">
            Cancel
          </button>
          <button type="submit" disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl text-sm font-semibold hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 transition-all shadow-lg shadow-cyan-500/20">
