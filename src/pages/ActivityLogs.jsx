...
const ACTION_COLORS = {
  ...
  PATIENT_ADDED:'text-cyan-400 bg-cyan-400/10', PATIENT_UPDATED:'text-blue-400 bg-blue-400/10',
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{total} total events logged</p>
        </div>
        <select value={action} onChange={e=>{setAction(e.target.value);setPage(1);}}
          className="bg-[#0d1526] border border-cyan-900/30 rounded-xl px-3 py-2 text-sm text-slate-300 focus:outline-none">
      ...
      <div className="bg-[#0d1526] border border-cyan-900/20 rounded-xl overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-cyan-900/10">
            {loading && <tr><td colSpan={6} className="py-10 text-center text-sm text-slate-500">Loading logs…</td></tr>}
            {!loading && logs.map((l,i)=>(
              <tr key={i} className="hover:bg-white/2 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="text-xs text-slate-300">{l.full_name||'System'}</div>
                  <div className="text-xs text-slate-600 capitalize">{l.role||'system'}</div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-xs text-slate-500">{l.entity_type||'—'} {l.entity_id?`#${l.entity_id}`:''}</td>
                <td className="px-4 py-3 text-xs text-slate-400 max-w-xs truncate">{l.details||'—'}</td>
                <td className="px-4 py-3 text-xs text-slate-600 font-mono">{l.ip_address||'—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {Math.ceil(total/20) > 1 && (
          <div className="flex justify-between items-center px-4 py-3 border-t border-cyan-900/20">
            <span className="text-xs text-slate-500">Page {page} of {Math.ceil(total/20)}</span>
            <div className="flex gap-2">
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} className="px-3 py-1.5 text-xs rounded-lg bg-white/5 disabled:opacity-30 text-slate-400">Prev</button>
              <button onClick={()=>setPage(p=>p+1)} disabled={page>=Math.ceil(total/20)} className="px-3 py-1.5 text-xs rounded-lg bg-white/5 disabled:opacity-30 text-slate-400">Next</button>
            </div>
          </div>
        )}
      </div>
